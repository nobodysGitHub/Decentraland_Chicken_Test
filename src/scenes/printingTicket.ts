...
export function howManyBoxes(boxesAmount: number){
    boxesNumber = boxesAmount
    howManyBoxesInGrid(boxesNumber)
}
...
export function printingTicket(gameCanvas: UICanvas, address: string)
{
    ...
    let num = Math.random() * (boxesNumber - 1) + 1; // num will never reach the max value (in this case 100) 
    log(num);
